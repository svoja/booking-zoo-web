  ...
  useEffect(() => {
    getPublicQuiz(id)
      .then((data) => {
        setQuiz(data);
        setAnswers(
          Object.fromEntries((data.questions || []).map((question) => [question.id, '']))
        );
      })
      .catch((err) => setError(err.message || 'ไม่พบ Quiz หรือ Quiz ยังไม่เปิดใช้งาน'))
    ...
    return (
      <div className="min-h-screen p-4">
        <div className={cardClass}>
          <p className="text-center text-slate-500">กำลังโหลด Quiz...</p>
        </div>
      </div>
    );
  }
