...
import { getPublicQuizResultsBySession } from '../api';
...
export default function QuizSessionPublicResults() {
  ...
  useEffect(() => {
    getPublicQuizResultsBySession(token)
      .then(setData)
  ...
  if (loading) {
    return <div className="min-h-screen p-4"><div className={cardClass}><p className="text-center text-slate-500">กำลังโหลดผลลัพธ์...</p></div></div>;
  }
  if (error || !data) {
    return <div className="min-h-screen p-4"><div className={cardClass}><p className="text-center text-rose-600">{error || 'ไม่พบผลลัพธ์'}</p></div></div>;
  }
  ...
  return (
    <div className="min-h-screen p-4">
      <div className={cardClass}>
        ...
        <p className="mb-1 text-sm font-semibold text-[#2d5a3a]">รอบ: {data.session?.name || '-'}</p>
        <p className="mb-3 text-sm text-slate-600">{data.quiz.description || 'สรุปผลคำตอบจากผู้ทำควิสในรอบนี้'}</p>
        <p className="mb-4 text-sm text-slate-600">จำนวนผู้ส่งคำตอบ: {data.submissions.length}</p>

        {data.submissions.length === 0 ? (
          <p className="text-slate-500">ยังไม่มีคำตอบในรอบนี้</p>
        ) : (
